/**
 * Interface representing an Asset in the system for the Portal.
 * Assets are records of external resources, typically stored in GCS or linked via URL.
 */

export interface Asset {
  ID: number;
  project_id?: number | null; // Optional foreign key for project
  account_id?: number | null; // Optional foreign key for account
  ...
  file?: File | null; // Used when uploading a new file (less relevant for portal read-only)
}

/**
 * Creates a new empty Asset object with default values for the Portal.
